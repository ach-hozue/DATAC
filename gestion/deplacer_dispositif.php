...
<html>
<head>
    <title>DATÀC – Déplacer un dispositif</title>
    <meta charset = "utf-8" />
...
<div class="container-fluid">
    ...
    <!--bannière grise avec choix de dispositif et catégorie de déplacement-->
    <div class="row rForm">
        <div class="col-sm-offset-1 col-sm-10">
            <form method = "post" id = "choix">
                <h3> Choississez le dispositif que vous voulez déplacer </h3>
                ...
                if(isset($_POST["defBis"]) && $_POST["defBis"] != 0) {
                    ...
                    <div class="form-group">
                        <select class="form-control" name="catBis1" id="niveau1"
                    ...
                    for ($i = 2; $i <= $NiveauBis; $i++) {
                        ...
                        if (isset($_POST["catBis$j"]) && $_POST["catBis$j"] != 0) {
                            ...
                            if (mysqli_num_rows($TabBis) != 0) {
                                ...
                                <div class="form-group">
                                    <select class="form-control" name="catBis<?php echo $i; ?>"
                                            id="niveau<?php echo $i; ?>" onChange="document.forms['choix'].submit();">
                                        <option value="0">--- Choisissez une catégorie ---</option>
                                        <?php
                                        while ($LecBis = mysqli_fetch_array($TabBis)) {
                                            // Si on a sélectionné une catégorie, on la laisse comme valeur par défaut
                ...
                <h3> Choississez la catégorie où vous voulez déplacer le dispositif </h3>
                <?php
                // Tableau pour ranger les identifiants des catégories sélectionnées
                $Selections = array();
                ...
                ?>
                <button class="btn btn-primary" type="submit" name="_deplacer">Déplacer</button>
            </form>
        </div>
    </div>
...
if(isset($_POST["_deplacer"]))
{
    ...
    {
        // Modification de la position du dispositif
        $RqtDis = "UPDATE dispositif SET id_cat_dis = $idCat WHERE id_dispositif = $idDispo";

        if(mysqli_query($BDD,$RqtDis))
        {
            // Message d'alerte
            ?>
            <script>
                alert("<?php echo htmlspecialchars('Le dispositif a bien été déplacé !', ENT_QUOTES); ?>");
